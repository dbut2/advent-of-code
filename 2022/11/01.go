...
func solve(input string) int {
	...
	for i := 0; i < 20; i++ {
		inspectionCounts = doRound(monkeys, inspectionCounts)
	}
...
func doRound(monkeys map[int]*monkey, inspections map[int]int) map[int]int {
	for i := 0; i < len(monkeys); i++ {
		m := monkeys[i]
		for j := range m.items {
			inspections[i]++

			m.items[j] = m.operation(m.items[j])
			m.items[j] /= 3
